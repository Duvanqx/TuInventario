...
def verCatalogo():
    if 'usuario_id' not in session:
        return redirect('/')
    catalogos = verCatalogos(session['usuario_id'])
...
def agregarCatalogo():
    if request.method == 'POST':
        ...
        agregarCatalogos(usuario, nombre, fecha_inicio, fecha_fin, fecha_pago, ruta_foto)
...
def editarCatalogo(id_catalogo):
    if request.method == 'POST':
        ...
        editarCatalogos(id_catalogo, ruta_foto, nombre, fecha_inicio, fecha_fin, fecha_pago)
        return redirect('/ver_catalogo')
    
    catalogo = informacionCatalogos(id_catalogo)
