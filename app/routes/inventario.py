...
import os
import uuid
from app.models.inventarios import agregarProductos, verProductos, editarProductos, obtenerProducto, elimarProductos
...
def verInventario():
    if 'usuario_id' not in session:
        return redirect('/')
    productos = verProductos(session['usuario_id'], current_app.mysql)
...
def agregarProducto():
    if request.method == 'POST':
        ...
        agregarProductos(usuario, nombre, categoria, cantidad, descripcion, precio, ruta_imagen, mysql)
...
def editarProducto(id_producto):
    if request.method == 'POST':
        ...
        editarProductos(nombre, categoria, cantidad, descripcion, precio, ruta_imagen, id_producto, mysql)
        return redirect('/ver_inventario')
    producto = obtenerProducto(id_producto,current_app.mysql)
...
def eliminarProducto(id_producto):
    elimarProductos(id_producto, current_app.mysql)
    return redirect('/ver_inventario')